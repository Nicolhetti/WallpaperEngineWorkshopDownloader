...
def run_command(pubfileid, save_location):
    ...
    if not os.path.isdir(target_directory):
        printlog(f"Invalid save location: {target_directory} does not exist or is incorrect.\n")
        return
    
    dir_option = f'-dir "{target_directory}\\{pubfileid}"'  # Ensure the directory path is correctly formatted for Windows
...
def run_commands(save_location):
    ...
    for link in links:
        if link:
            ...
            if match:
                run_command(match.group(0), save_location)
            else:
...
def start_thread():
    threading.Thread(target=run_commands, args=(save_location,)).start()
